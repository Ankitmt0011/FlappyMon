...
async function connectWallet() {
  try {
    const address = await connectWarpcast();
    userAddress = address;
    console.log("Connected wallet:", userAddress);
    return address;
  } catch (error) {
    alert("Warpcast wallet not available.");
    return null;
  }
}

const GAME_ENTRY_FEE = "0.03";
const MONAD_CHAIN_ID = "0x3151c"; // Monad testnet
...
async function payEntryFee() {
  ...
  try {
    ...
    console.log("Fee paid:", txHash);
    return true;
  } catch (error) {
    console.error("Payment failed:", error);
    alert("Payment failed. Make sure you're on Monad testnet and have test MON.");
    return false;
  }
}
...
async function saveScore(score) {
  ...
  try {
    ...
    if (score > current) {
      ...
      console.log("Lower than current high score.");
    }
  } catch (err) {

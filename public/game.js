...
let userAddress = null; const canvas = document.getElementById("gameCanvas"); const ctx = canvas.getContext("2d");
...
// Images const birdImg = new Image(); birdImg.src = 'icon.png';

const bgImg = new Image(); bgImg.src = 'background.jpg';

// Sound Effects (preloaded) const tapSound = new Audio("tap.wav"); const gameOverSound = new Audio("gameover.wav"); const clickSound = new Audio("click.wav");
...
let gravity = 0.5; let pipeSpeed = 3; let pipes = []; let score = 0;
...
// Event Listeners startBtn.addEventListener("click", async () => { playSound(clickSound); userAddress = await connectWarpcast(); if (!userAddress) return;
...
const GAME_ENTRY_FEE = "0.03"; const MONAD_CHAIN_ID = "0x3151c"; const PROJECT_WALLET_ADDRESS = "0xfc8e4ffEC914D567460AC549a3CD642981A43674";

async function payEntryFee() { if (!userAddress) return;

try { await window.ethereum.request({ method: 'wallet_switchEthereumChain', params: [{ chainId: MONAD_CHAIN_ID }] });
...
} catch (error) { console.error("Transaction failed:", error); alert("Payment failed. Make sure you’re connected to Monad testnet and have test MON."); return false; } }
...
async function initContract() { const provider = new ethers.providers.Web3Provider(window.ethereum); const signer = provider.getSigner(); contract = new ethers.Contract(CONTRACT_ADDRESS, ABI, signer); }

initContract();

async function saveScore(score) { if (!contract || !userAddress) return;

try { const current = await contract.getHighScore(userAddress); if (score > current) { const tx = await contract.submitScore(score); await tx.wait(); console.log("Score updated on-chain!"); } else { console.log("Score not higher than existing."); } } catch (err) { console.error("Error saving score:", err); } }
